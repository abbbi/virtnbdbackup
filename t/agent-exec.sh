...
execute_qemu_command() {
    ...
    if [ $exit_code -ne 0 ]; then
        echo "Error executing QEMU agent command: $exec_result" >&2
        return $exit_code
    fi
    pid=$(echo "$exec_result" | jq -r '.return.pid //empty')
    if [ -z "$pid" ]; then
        echo "Failed to retrieve PID from guest-exec response." >&2
        return 1
    fi
    ...
    while true; do
        ...
        if [ $? -ne 0 ]; then
            echo "Error fetching guest-exec-status: $status_result" >&2
            return 1
        fi
